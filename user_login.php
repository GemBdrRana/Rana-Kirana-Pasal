...
if(isset($_POST['submit'])){
   ...
   if($select_user->rowCount() > 0){
      ...
      setcookie('user_id', $row['id'], time() + 60*60*24*1000, '/');
...
<html lang="en">
<head>
  <link rel="icon" href="images/register.ico" size="25*25" type="image/x-icon" />
...
<section class="form-container">
   ...
   <form action="" method="post" style="border:none;box-shadow: rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px, rgba(10, 37, 64, 0.35) 0px -2px 6px 0px inset;">
      <h3>login now</h3>
      <input type="email" name="email" required placeholder="enter your email" maxlength="50"  class="box" oninput="this.value = this.value.replace(/\s/g, '')" id="email">
      <input type="password" name="pass" required placeholder="enter your password" maxlength="20"  class="box" oninput="this.value = this.value.replace(/\s/g, '')">
      <p><input type="checkbox" value="lsRememberMe" id="rememberMe" /> Remember me
  </p>
      <input type="submit" value="login now" class="btn" name="submit" style=" background: -webkit-linear-gradient(right, #a6f77b, #2dbd6e);
         color: white;" onclick="lsRememberMe()">
